...
path_out = os.path.join(my_path, "../data/yelp/BJRI/")
print(path_out)

yelp = pd.read_csv(path_out + "bjs-restaurant-and-brewhouse-new-braunfels-2.csv")
...
my_path = os.path.abspath(os.path.dirname(__file__))
path_out = os.path.join(my_path, "../data/pos/")
print(path_out)

# Normal Nouns Functions

kas = firstme(best, worst, "good", "bad")
kas.to_csv(path_out + "good_bad.csv", index=False)

kas = firstme(worst, best, "bad", "good")
kas.to_csv(path_out +"bad_good.csv", index=False)
...
guud_p.to_csv(path_out +"good_bad_pro.csv", index=False)

beet_p = firstmep(worst, best, "worst", "best")

beet_p.to_csv(path_out +"bad_good_pro.csv", index=False)