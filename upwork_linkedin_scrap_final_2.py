...
for restaurant in rest_rem_list:
	...
	for com in com_list:
		if 'name' in com.keys():
			matching_ratio.append(SequenceMatcher(None, com['name'].lower(), restaurant.lower()).ratio())
		...
		if len(emp_list) == 0:
			...
			for emp in emp_list:
				if 'firstName' in emp.keys():
					emp['urn'] = emp['objectUrn']
					employees.append(emp)
				elif 'location' in emp.keys():
					emp['urn'] = emp['backendUrn']
					employees.append(emp)
...
for key,value in emp_details_dict1.items():
	emp_list = []
	for val in value:
		emp_list.append(dict([(b,c) for b, c in val.items() if b in ['firstName','lastName','occupation','educations','location','industry']]))
